...
class MusicDataBunch(DataBunch):
    ...
    def from_files(
        ...
        **kwargs
        ...
        src = (
            list_cls(
                items=files, path=path, processor=processors, vocab=vocab
            )
...
class MusicItemList(ItemList):
    ...
    def __init__(
        self, items: Iterator, vocab: MusicVocab = None, **kwargs
    ):
...
class OpenNPFileProcessor(PreProcessor):
    ...
    def process_one(self, item):
        return (
            np.load(item, allow_pickle=True)
...
class Midi2ItemProcessorVocab(Midi2ItemProcessor):
    "Fixes out-of-range embedding"
...
class MusicPreloader(Callback):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.transpose_range, self.transpose_p = (
            transpose_range,
            transpose_p,
        )
        ...
        if self.ite_len is None:
            if self.lengths is None:
                self.lengths = np.array(
                    [len(item) for item in self.dataset.x]
                )
            self.totalToks = self.lengths.sum()
            self.ite_len = (
                self.bs
                * int(math.ceil(self.totalToks / (self.bptt * self.bs)))
    ...
    def allocate_buffers(self):
        ...
        self.batch = np.zeros(
            (self.bs, self.bptt + self.y_offset) + buffer_len,
            dtype=np.int64,
        )
